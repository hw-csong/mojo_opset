...
def micro_kernel_fwd(
    ...
    mask_kv = (offset_c + tl.arange(0, BLOCK_C)) < offset_c_ed
    block_k = tl.load(ptr_k, mask=mask_kv[:, None], other=0.0)
    block_k = tl.trans(block_k)
    block_s = tl.dot(block_q, block_k) * scale
    block_v = tl.load(ptr_v, mask=mask_kv[:, None], other=0.0)
    if block_mask is not None:
        block_s += block_mask
    elif tile_r_mask:
        block_s += tl.where(mask_kv[None, :], 0.0, -1e6)
...
def kernel_da_bwd_kv_ur2(
    ...
    for idx_seq in range(num_seqs):
        ...
        for task_id in range(
            ...
            # block_dk = tl.full([BLOCK_C, H], 0.0, dtype=HIGH_TYPE)
            # block_dv = tl.full([BLOCK_C, H], 0.0, dtype=HIGH_TYPE)
            block_dk = tl.load(ptr_dk, mask=mask_kv, other=0.0).to(HIGH_TYPE)
            block_dv = tl.load(ptr_dv, mask=mask_kv, other=0.0).to(HIGH_TYPE)
...
def dllm_attention_up_bwd_impl(
    ...
    kernel_da_bwd_kv_ur2[(num_cores,)](
        q,
        k,
        v,
        do,
