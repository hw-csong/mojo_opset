...
def micro_kernel_fwd_1to2(
    ...
    ptr_k = (
        k
        + (idx_n // GROUP_SIZE) * STRIDE_K_N
        + (offset_c + tl.arange(0, BLOCK_KV))[:, None] * STRIDE_K_S
        + idx_h[None, :] * STRIDE_K_H
    )
    ptr_v = (
        v
        + (idx_n // GROUP_SIZE) * STRIDE_V_N
        + (offset_c + tl.arange(0, BLOCK_KV))[:, None] * STRIDE_V_S
        + idx_h[None, :] * STRIDE_V_H
    )

    mask_kv = (offset_c + tl.arange(0, BLOCK_KV))[:, None] < offset_c_ed
    block_k = tl.load(ptr_k, mask=mask_kv, other=0.0)
    block_k = tl.trans(block_k)
    block_s = tl.dot(block_q, block_k)

    # vec 1 to 2 save block_s to GM
    offs_buf = tl.arange(0, BLOCK_Q)[:, None] * STRIDE_BUF_R + tl.arange(0, BLOCK_KV)[None, :] * STRIDE_BUF_C
    tl.store(tbuf + offs_buf, block_s)
    block_v = tl.load(ptr_v, mask=mask_kv, other=0.0)
    # load sub block_s to vec
    sub_vec_id = al.sub_vec_id()
    SUB_BLOCK_Q = BLOCK_Q // al.sub_vec_num()
    offs_sub_buf = (
        (sub_vec_id * SUB_BLOCK_Q + tl.arange(0, SUB_BLOCK_Q)[:, None]) * STRIDE_BUF_R
        + tl.arange(0, BLOCK_KV)[None,:] * STRIDE_BUF_C
    )
    ...
    block_s_sub = tl.exp(block_s_sub - block_m_1[:, None])
    block_l_1 = tl.exp(block_m - block_m_1) * block_l + tl.sum(block_s_sub, axis=1)
    # store sub block_s to GM
    tl.store(tbuf + offs_sub_buf, block_s_sub)
    block_o = tl.exp(block_m - block_m_1)[:, None].to(LOW_TYPE) * block_o
    # load full block_s to cube
    block_s = tl.store(tbuf + offs_buf)
    sv = tl.dot(block_s.to(LOW_TYPE), block_v).to(
        LOW_TYPE
    )
    block_o = block_o + sv
    ...
    for idx_seq in range(num_seqs):
        ...
        for task_id in range(
            ...
            for idx_tile_r in range(idx_r * BLOCK_R // BLOCK_C * BLOCK_C // BLOCK_R, idx_r):
                block_o, block_m, block_l = micro_kernel_fwd_1to2(
                    block_q,
                    k,
                    v,
                    block_o,
...
def dllm_attention_up_fwd_impl(
    ...
    tbuf = torch.zeros((num_cores, FWD_BLOCK_C, FWD_BLOCK_C), device=q.device, dtype=torch.float32)
    ...
    kernel_da_fwd_u[(num_cores,)](
        ...
        tubf=tbuf,
        STRIDE_BUF_P=tbuf.stride(0),
        STRIDE_BUF_R=tbuf.stride(1),
        STRIDE_BUF_C=tbuf.stride(2),
    )
